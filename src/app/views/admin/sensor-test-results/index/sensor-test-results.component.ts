...
export class SensorTestResultsComponent extends BaseEntity<SensorTestResultsEntity> {
  ...
  override pdfPrefix: string = "api/sensor-test-results/pdfmultiple";

  override async loadDetailData(id: number): Promise<any> {

    const factuuropdracht = this.data.find(sensorTestResult => sensorTestResult.id === id)?.factuuropdracht;

    try {

      await this.sensorTestResultsService.fetch({
        filter: {
          and: [
            {
              factuuropdracht: {
                equalTo: factuuropdracht
              }
            }
          ]
        }
      }, { fetchPolicy: 'no-cache' }).subscribe(result => {
        this.retrieveFile(factuuropdracht, result?.data?.allSensorTestResultsEntities?.nodes as SensorTestResultsEntity[] || null);
      });
  ...
  async retrieveFile(id: number, body: any) {
    this.fileService.downloadPdfWithBody(this.pdfPrefix, id, body).subscribe((data: Blob) => {
      ...
      link.href = fileUrl;
      link.download = `sensor_test_results_${id}.pdf`; // Specify the desired file name
