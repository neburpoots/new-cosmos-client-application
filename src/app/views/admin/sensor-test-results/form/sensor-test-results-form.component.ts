...
import { AllCalibrationGasesNoPaginationGQL, AllSensorBaseTypesNoPaginationGQL, AllSensorTypesNoPaginationGQL, AllSensorsGQL, AllUsersNoPaginationGQL, CalgasEntity, CreateMembraneGQL, CreateSensorBaseTypeGQL, CreateSensorOrderGQL, CreateSensorTestResultGQL, Principle, Sensor, SensorBaseType, SensorBaseTypeInput, SensorIndex, SensorTestResultInput, SensorTestResultsEntity, SensorType, SensorTypeInput, SensorTypesIndex, UpdateMembraneGQL, UpdateSensorBaseTypeGQL, UpdateSensorTestResultGQL, User } from '../../../../../generated/graphql';
...
export class SensorTestResultsFormComponent extends BaseFormComponent<SensorTestResultInput> {
    ...
    createSensorOrderService: CreateSensorOrderGQL;
    ...
    constructor(
        ...
        this.createSensorOrderService = createSensorOrderService;
    ...
    handleCreateSensorOrder(selectedSensor: SensorIndex) {
        this.createSensorOrderService.mutate({
            body: {
                factuuropdracht: this.myForm.value.order,
            ...
            this.selectedSensor = {
                ...selectedSensor,
                factuuropdracht: this.myForm.value.order,
                sensorOrderId: result?.data?.createSensorOrder?.sensorOrder?.id || 0
            }
    ...
    override onSubmit(): void {
        try {
            ...
            if (this.myForm.valid) {
                ...
                if (this.selectedSensor && !this.selectedSensor?.factuuropdracht) {
                    this.handleCreateSensorOrder(this.selectedSensor);
                } else {


                    if (this.id) {
                        this.update(this.createDto());
                    } else {
